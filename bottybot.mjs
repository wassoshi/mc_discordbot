...
const isBlacklistedName = (s) =>
  typeof s === 'string' && /b[^a-zA-Z0-9]*(?:o|0)[^a-zA-Z0-9]*n[^a-zA-Z0-9]*n[^a-zA-Z0-9]*(?:a|4|@)/i.test(s);
...
function createWeb3Provider() {
  ...
  const isSocketOpen = () => {
    const c = wsProvider?.connection;
    return !!c && c.readyState === 1; // 1 = OPEN
  };
  ...
  function startPing() {
    ...
    pingInterval = setInterval(() => {
      ...
      try {
        wsProvider.send(
          { jsonrpc: '2.0', id: Date.now(), method: 'net_version', params: [] },
          (err) => {
            if (err) {
  ...
  function startHealthCheckOnce() {
    ...
    healthInterval = setInterval(() => {
      ...
      console.log(`WebSocket health check: Connection is ${open ? 'open' : 'closed'}`);
      if (!open) reconnectIfNeeded();
    }, 600000);
  }

  function safeCloseCurrent() {
    try {
      if (wsProvider?.connection && wsProvider.connection.readyState === 1) {
        wsProvider.connection.close(1000, 'reconnect');
      }
    } catch (e) {
  ...
  function setupWebSocketProvider() {
    ...
    const url = `wss://eth-mainnet.g.alchemy.com/v2/${process.env.SALES_ALCHEMY_PROJECT_ID}`;
    wsProvider = new Web3.providers.WebsocketProvider(url);

    currentWsProvider = wsProvider;
  ...
  function reconnectIfNeeded() {
    ...
    console.log(`Reconnection attempt #${retryCount} in ${Math.round(delay / 1000)} seconds...`);
...
function runSalesBot() {
  ...
  async function announceOldWrapperSale(tokenId, ethPrice, transactionUrl, paymentToken, protocolAddress, buyerAddress) {
    ...
    try {
      ...
      if (isNaN(formattedEthPrice)) throw new Error('Invalid ethPrice');
    } catch (error) {
    ...
    const messageText =
      ...
      `[${displayBuyerAddress}](https://chainstation.mooncatrescue.com/owners/${buyerAddress}) ` +
      `for ${formattedEthPrice} ${currency} ($${usdPrice})\n\n[ ${classification} ]`;
    ...
    while (transferQueue.length > 0) {
      ...
      try {
        ...
        if (receipt && receipt.status) {
          ...
          if (!isProcessingSales) processSalesQueue();
        } else {
  ...
  // Re-subscribing transfers after reconnect
  let salesSubs = [];
  let lastSeenTransferBlock = null;

  function unsubscribeAllSales() {
  ...
  function transfersFromBlock() {
    if (!lastSeenTransferBlock) return 'latest';
    return Math.max(lastSeenTransferBlock - 5, 0);
  }

  function subscribeSalesTransfers() {
    console.log('Subscribing SalesBot Transfer listeners...');
    unsubscribeAllSales();
    ...
    const subA = mooncatsContract.events.Transfer({ fromBlock: transfersFromBlock() })
      .on('data', (event) => {
        if (event?.blockNumber != null) lastSeenTransferBlock = event.blockNumber;
        ...
        if (transferQueue.length === 1) processTransferQueue();
      })
      .on('error', (error) => {
        console.error('Error in MoonCats transfer event listener:', error);
      });

    const subB = oldWrapperContract.events.Transfer({ fromBlock: transfersFromBlock() })
      .on('data', (event) => {
        if (event?.blockNumber != null) lastSeenTransferBlock = event.blockNumber;
    ...
    salesSubs.push(subA, subB);
  }

  wsEvents.on('ws:connect', () => {
    console.log('WS connected. Re-subscribing SalesBot listeners.');
    subscribeSalesTransfers();
  });
...
function runListingBot() {
  ...
  async function getRealTokenIdFromWrapper(tokenId) {
    const provider2 = new AlchemyProvider('homestead', process.env.LISTING_ALCHEMY_PROJECT_ID);
    const contract = new Contract(OLD_WRAPPER_CONTRACT_ADDRESS, OLD_WRAPPER_CONTRACT_ABI, provider2);
...
async function runNameBot() {
  ...
  // Re-subscribing naming listener after reconnect
  let nameSub = null;
  let lastSeenNameBlock = null;

  function unsubscribeName() {
    if (!nameSub) return;
    try { nameSub.unsubscribe(); } catch (e) {}
    nameSub = null;
  }

  function namesFromBlock() {
    if (!lastSeenNameBlock) return 'latest';
    return Math.max(lastSeenNameBlock - 5, 0);
  }

  function subscribeNaming() {
    console.log('Subscribing NameBot CatNamed listener...');
    unsubscribeName();

    const moonCatsNamingContract = new web3.eth.Contract(moonCatsNamingAbi, MOONCATS_NAMING_CONTRACT_ADDRESS);

    nameSub = moonCatsNamingContract.events.CatNamed({ fromBlock: namesFromBlock() })
      .on('data', async (event) => {
        if (event?.blockNumber != null) lastSeenNameBlock = event.blockNumber;
  ...
  wsEvents.on('ws:connect', () => {
    console.log('WS connected. Re-subscribing NameBot listener.');
    subscribeNaming();
  });
...
function shutdown() {
  ...
  try {
    if (currentWsProvider?.connection) {
      currentWsProvider.connection.close(1000, 'shutdown');
    }
  } catch (e) {}
  process.exit(0);
}
