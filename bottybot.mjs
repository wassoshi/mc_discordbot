...
function runSalesBot() {
    ...
    async function getOldWrapperImageAndDetails(tokenId) {
        try {
            ...
            return { imageUrl: `https://assets.coingecko.com/coins/images/36766/large/mooncats.png?1712283962`, name: null, rescueIndex: null, realTokenIdHex: null, isNamed: false };
        }
    }
    ...
    async function fetchSaleDataFromOpenSea(tokenId, sellerAddress) {
        try {
            ...
            return { tokenId, ethPrice, transactionUrl, payment: paymentToken, fromAddress: saleEvent.seller, toAddress: saleEvent.buyer, protocolAddress: saleEvent.protocol_address, saleSellerAddress: sellerAddress };
        } catch (error) {
            return null;
        }
    }
    ...
    async function processSalesQueue() {
        while (salesQueue.length > 0) {
            ...
            try {
                ...
                if (saleData) {
                    await announceMoonCatSale(saleData.tokenId, saleData.ethPrice, saleData.transactionUrl, saleData.payment, saleData.protocolAddress, saleData.toAddress);
                    await new Promise(resolve => setTimeout(resolve, DISCORD_MESSAGE_DELAY_MS));
                }
            } catch (error) {
            }
    ...
    mooncatsContract.events.Transfer({
        ...
        transferQueue.push({ tokenId: event.returnValues.tokenId, transactionHash: event.transactionHash, sellerAddress: event.returnValues.from.toLowerCase() });
...
function runListingBot() {
    ...
    async function getOldWrapperImageAndDetails(tokenId) {
        try {
            ...
            return { imageUrl: `https://assets.coingecko.com/coins/images/36766/large/mooncats.png?1712283962`, name: `Wrapped MoonCat #${tokenId}` };
        }
    }
    ...
    async function fetchListingsFromOpenSea(initialRun = false) {
        try {
            ...
            const headers = { 'X-API-KEY': OPENSEA_API_KEY, 'Accept': 'application/json' };
            const [responseMoonCats, responseOldWrapper] = await Promise.all([fetch(openseaAPIUrlMoonCats, { headers }), fetch(openseaAPIUrlOldWrapper, { headers })]);
